...
export default function MovieModal({ movie, onClose }) {
  ...
  const [relatedToggle, setRelatedToggle] = useState<"recommended" | "similar">(
    "recommended",
  );
    ...
    <div className="fixed inset-0 bg-black bg-opacity-80 flex items-center justify-center z-50 p-4 overflow-auto">
      <div className="bg-white rounded-lg w-full max-w-3xl max-h-[500px] overflow-y-auto relative p-4 md:p-6">
        ...
        <div className={"flex gap-4 mb-4 border-b pb-2"}>
          {["Details", "Trailer", "Reviews", "Cast & Crew", "Related"].map(
            (t) => (
              <button
                key={t}
                onClick={() => setTab(t)}
          ...
          <div className="flex flex-col gap-6 md:flex-row mt-8">
            ...
            <div className="flex-1">
              ...
              <div className="text-sm text-gray-600 space-y-1">
                ...
                {fullMovieDetails?.production_countries?.length > 0 && (
                  <p>
                    ...
                    {fullMovieDetails.production_countries
                      .map((c) => c.name)
                      .join(", ")}
                  </p>
                )}
                ...
                {fullMovieDetails?.runtime && (
                  <p>
                    ...
                    {Math.floor(fullMovieDetails.runtime / 60)}h{" "}
                ...
                {fullMovieDetails?.budget > 0 && (
                  <p>
                    ...
                    {fullMovieDetails.budget.toLocaleString()}
                  </p>
                )}
                ...
                {fullMovieDetails?.revenue > 0 && (
                  <p>
                    ...
                    {fullMovieDetails.revenue.toLocaleString()}
                  </p>
                )}
                <p>
              ...
              {fullMovieDetails?.production_companies?.length > 0 && (
                <div className="mt-6">
                  <h3 className="text-lg font-semibold mb-2 border-b pb-1 text-gray-800">
            ...
            <div>
                ...
                {credits.cast.map((actor) => (
                  <div key={actor.id} className={"text-center w-32"}>
                    <img
                      ...
                      alt={actor.name}
                      className={
                        "w-full h-48 object-cover rounded mb-1 cursor-pointer"
                      }
                      onClick={() => setSelectedPersonId(actor.id)}
                    />
                    <p
                      className={
                        "font-medium text-sm cursor-pointer text-blue-600 hover:underline"
                      }
                      onClick={() => setSelectedPersonId(actor.id)}
                    >
                      {actor.name}
              ...
              <div>
                ...
                <ul className="space-y-1 text-sm text-gray-700">
                  {["Director", "Producer", "Writer", "Composer"].map(
                    (role) => {
            ...
            >
              {(relatedToggle === "recommended"
                ...
                <div
                  ...
                  onClick={() => setSeconddaryMovies(movie)}
                >
                  <img
        ...
        {seconddaryMovies && (
          <SecondaryMovieModal
            movie={seconddaryMovies}
            onClose={() => setSeconddaryMovies(null)}
          />
        )}
      </div>
    </div>
